...
enum ZerocoinSpendStatus {
    ...
    ZSYNX_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZSYNXOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzsynxAuto) const;
    ...
    */
    ...
    CzSYNXWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzSYNXTracker> zsynxTracker;
    ...
    void setZWallet(CzSYNXWallet* zwallet)
    {
        zwalletMain = zwallet;
        zsynxTracker = std::unique_ptr<CzSYNXTracker>(new CzSYNXTracker(strWalletFile));
    }

    CzSYNXWallet* getZWallet() { return zwalletMain; }
    ...
    /** zSYNX reset */
    boost::signals2::signal<void()> NotifyzSYNXReset;
