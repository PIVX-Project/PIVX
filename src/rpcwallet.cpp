...
// Copyright (c) 2015-2018 The XVIP developers
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "      amount,                 (numeric) The amount in SYNX\n"
...
UniValue movecmd(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw std::runtime_error(
            ...
            "3. amount            (numeric, required) Quantity of SYNX to move between accounts.\n"
            ...
            "\nMove 0.01 SYNX from timotei to akiko with a comment\n" +
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in SYNX. This is negative and only available for the \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in SYNX. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in SYNX\n"
            ...
            "      \"amount\" : x.xxx                  (numeric) The amount in SYNX\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee configuration, set in SYNX/kB\n"
            "}\n"
...
UniValue getzerocoinbalance(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getzerocoinbalance\n"
            "\nReturn the wallet's total zSYNX balance.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nResult:\n"
            "amount         (numeric) Total zSYNX balance.\n"
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "\nList all zSYNX mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "    \"zSYNX ID\": \"xxx\",   (string) Pubcoin in hex format.\n"
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zsynxTracker->ListMints(true, fMatureOnly, true);
    ...
    if (fVerbose) {
        for (const CMintMeta& m : setMints) {
            ...
            objMint.push_back(Pair("zSYNX ID", m.hashPubcoin.GetHex()));     // PubCoin
...
UniValue listzerocoinamounts(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zsynxTracker->ListMints(true, true, true);
...
UniValue listspentzerocoins(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "listspentzerocoins\n"
            "\nList all the spent zSYNX mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue mintzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nMint the specified zSYNX amount\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. amount      (numeric, required) Enter an amount of Synx to convert to zSYNX\n"
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 5 || params.size() < 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"value\": amount,         (numeric) Value in SYNX.\n"
            "      \"address\": \"xxx\"         (string) SYNX address or \"zerocoinmint\" for reminted change.\n"
            "    }\n"
            "    ,...\n"
            "  ]\n"
            "}\n"
    ...
    return DoZsynxSpend(nAmount, fMintChange, fMinimizeChange, nSecurityLevel, vMintsSelected, address_str);
}
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nSpend zSYNX mints to a SYNX address.\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    return DoZsynxSpend(nAmount, false, true, 100, vMintsSelected, address_str);
}


extern UniValue DoZsynxSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, const int nSecurityLevel, vector<CZerocoinMint>& vMintsSelected, std::string address_str)
{
...
UniValue resetmintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzSYNXTracker* zsynxTracker = pwalletMain->zsynxTracker.get();
    set<CMintMeta> setMints = zsynxTracker->ListMints(false, false, true);
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzSYNXTracker* zsynxTracker = pwalletMain->zsynxTracker.get();
    set<CMintMeta> setMints = zsynxTracker->ListMints(false, false, false);
    ...
    for (CZerocoinSpend spend : listUnconfirmedSpends) {
        for (auto& meta : setMints) {
            if (meta.hashSerial == GetSerialHash(spend.GetSerial())) {
                zsynxTracker->SetPubcoinNotUsed(meta.hashPubcoin);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || params.empty() || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"denomination\"         (integer, optional) Export a specific denomination of zSYNX\n"
            ...
            "    \"v\": version,   (numeric) The version of the zSYNX\n"
            "    \"k\": \"privkey\"  (string) The zSYNX private key (V2+ zSYNX only)\n"
            "  }\n"
            "  ,...\n"
            "]\n"
    ...
    CzSYNXTracker* zsynxTracker = pwalletMain->zsynxTracker.get();
    set<CMintMeta> setMints = zsynxTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue importzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() == 0)
        throw runtime_error(
            ...
            "  \"value\": amount    (numeric) The total zSYNX value of zerocoin mints that were added\n"
            "}\n"
    ...
    for (unsigned int idx = 0; idx < arrMints.size(); idx++) {
        ...
        pwalletMain->zsynxTracker->Add(mint, true);
        count++;
...
UniValue reconsiderzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw runtime_error(
            "reconsiderzerocoins\n"
            "\nCheck archived zSYNX list to see if any mints were added to the blockchain.\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue setzsynxseed(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 1)
        throw runtime_error(
            "setzsynxseed \"seed\"\n"
            "\nSet the wallet's deterministic zsynx seed to a specific value.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"seed\"        (string, required) The deterministic zsynx seed.\n"

            "\nResult\n"
            "\"success\" : b,  (boolean) Whether the seed was successfully set.\n"

            "\nExamples\n" +
            HelpExampleCli("setzsynxseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
            HelpExampleRpc("setzsynxseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
    ...
    CzSYNXWallet* zwallet = pwalletMain->getZWallet();
...
UniValue getzsynxseed(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw runtime_error(
            "getzsynxseed\n"
            ...
            "\nResult\n"
            "\"seed\" : s,  (string) The deterministic zSYNX seed.\n"

            "\nExamples\n" +
            HelpExampleCli("getzsynxseed", "") + HelpExampleRpc("getzsynxseed", ""));
...
UniValue generatemintlist(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 2)
        throw runtime_error(
            "generatemintlist\n"
            "\nShow mints that are derived from the deterministic zSYNX seed.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"  : n,  (numeric) Which sequential zSYNX to start with.\n"
            "2. \"range\"  : n,  (numeric) How many zSYNX to generate.\n"
    ...
    CzSYNXWallet* zwallet = pwalletMain->zwalletMain;
...
UniValue dzsynxstate(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 0)
        throw runtime_error(
                "dzsynxstate\n"
                        "\nThe current state of the mintpool of the deterministic zSYNX wallet.\n" +
                HelpRequiringPassphrase() + "\n"
    ...
    obj.push_back(Pair("dzsynx_count", nCount));
...
void static SearchThread(CzSYNXWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZSYNX(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
UniValue searchdzsynx(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 3)
        throw runtime_error(
            "searchdzsynx\n"
            "\nMake an extended search for deterministically generated zSYNX that have not yet been recognized by the wallet.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"       (numeric) Which sequential zSYNX to start with.\n"
            "2. \"range\"       (numeric) How many zSYNX to generate.\n"
            "3. \"threads\"     (numeric) How many threads should this operation consume.\n"

            "\nExamples\n" +
            HelpExampleCli("searchdzsynx", "1, 100, 2") + HelpExampleRpc("searchdzsynx", "1, 100, 2"));
    ...
    boost::thread_group* dzsynxThreads = new boost::thread_group();
    ...
    for (int i = 0; i < nThreads; i++) {
        ...
        nPrevThreadEnd = nEnd;
        dzsynxThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
    }

    dzsynxThreads->join_all();

    zwallet->RemoveMintsFromPool(pwalletMain->zsynxTracker->GetSerialHashes());
