...
{
    return zsynxTracker->HasSerial(bnSerial);
}
...
CAmount CWalletTx::GetAnonymizableCredit(bool fUseCache) const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        if (fMasterNode && CMasternode::CheckCollateral(COutPoint(hashTx, i)) == CMasternode::COLLATERAL_OK) continue; // do not count MN-like outputs
        // SYNX END 
...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        else if (fMasterNode && CMasternode::CheckCollateral(COutPoint(hashTx, i)) == CMasternode::COLLATERAL_OK) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
        }
        // SYNX END 
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    bool fCheckZSYNX = GetBoolArg("-zapwallettxes", false);
    if (fCheckZSYNX)
        zsynxTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zsynx
            if (fCheckZSYNX && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
CAmount CWallet::GetZerocoinBalance(bool fMatureOnly) const
{
    if (fMatureOnly) {
        ...
        CAmount nBalance = 0;
        vector<CMintMeta> vMints = zsynxTracker->GetMints(true);
    ...
    return zsynxTracker->GetBalance(false, false);
}
...
{
    return zsynxTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        set<CMintMeta> setMints = zsynxTracker->ListMints(true, true, true);
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_DENOMINATED) {
                    ...
                    if (found && fMasterNode) found = CMasternode::CheckCollateral(COutPoint(pcoin->GetHash(), i)) != CMasternode::COLLATERAL_OK;
                    // SYNX END  
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount)
{
    ...
    if (GetBoolArg("-synxstake", true)) {
        for (const COutput &out : vCoins) {
            ...
            std::unique_ptr<CSynxStake> input(new CSynxStake());
    ...
    //zSYNX
    if (GetBoolArg("-zsynxstake", true) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        //Only update zSYNX set once per update interval
        bool fUpdate = false;
        ...
        set<CMintMeta> setMints = zsynxTracker->ListMints(true, true, fUpdate);
        for (auto meta : setMints) {
            ...
            if (meta.nHeight < chainActive.Height() - Params().Zerocoin_RequiredStakeDepth()) {
                std::unique_ptr<CZSynxStake> input(new CZSynxStake(meta.denom, meta.hashStake));
                listInputs.emplace_back(std::move(input));
            }
        }
    }
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZsynxBalance = GetZerocoinBalance(false);
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax) const
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if(fMasterNode && CMasternode::CheckCollateral(COutPoint(out.tx->GetHash(), out.i)) == CMasternode::COLLATERAL_OK) continue;
        // SYNX END
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 SYNX collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    vector<pair<CScript, CAmount> > vecSend;
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 SYNX collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 SYNX.");
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(stakeInput.get(), nBits, block.GetBlockTime(), nTxNewTime, hashProofOfStake)) {
            ...
            //Masternode payment
            FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZSYNX());
            ...
            //Mark mints as spent
            if (stakeInput->IsZSYNX()) {
                CZSynxStake* z = (CZSynxStake*)stakeInput.get();
...
bool CWallet::CreateZSYNXOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZSYNX(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZSYNXOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zsynx output", __func__);
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
        ...
        if (!zerocoinSelected.GetKeyPair(key))
            return error("%s: failed to set zSYNX privkey mint version=%d", __func__, nVersion);
    ...
    try {
        ...
        //Add the coin spend into a Syndicate transaction
        ...
        if (IsSerialKnown(spend.getCoinSerialNumber())) {
            ...
            if (!zsynxTracker->HasSerialHash(hashSerial))
                return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

            CMintMeta meta = zsynxTracker->Get(hashSerial);
            meta.isUsed = true;
            if (!zsynxTracker->UpdateState(meta))
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CDeterministicMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CBitcoinAddress* address)
{
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zSYNX"), nStatus);
        return false;
    }
    ...
    if (vSelectedMints.empty()) {
        setMints = zsynxTracker->ListMints(true, true, true); // need to find mints to spend
        if(setMints.empty()) {
    ...
    for (const auto& mint : vSelectedMints) {
        if (mint.GetVersion() < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            if (nSecurityLevel < 100) {
                nStatus = ZSYNX_SPEND_V1_SEC_LEVEL;
                receipt.SetStatus(_("Version 1 zSYNX require a security level of 100 to successfully spend."), nStatus);
                return false;
            }
        }
    }
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zsynxTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zsynxTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zsynxTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            zsynxTracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

string CWallet::GetUniqueWalletBackupName(bool fzsynxAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzsynxAuto ? "-autozsynxbackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}
...
void CWallet::ZSynxBackupWallet()
{
    ...
    if(!GetArg("-zsynxbackuppath", "").empty()) {
        filesystem::path customPath(GetArg("-zsynxbackuppath", ""));
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo)
{
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zsynxTracker->SetPubcoinNotUsed(hashPubcoin);
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zsynxTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zsynxTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
...
{
    CMintMeta meta;
    if (!zsynxTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zsynxTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zsynxTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zsynxTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zsynxTracker->UpdateState(meta);
    } else {
