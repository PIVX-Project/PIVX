...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZSynxStake(spend));
    } else {
        ...
        CSynxStake* synxInput = new CSynxStake();
        synxInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(synxInput);
    }
