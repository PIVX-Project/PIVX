        ...
        )
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(pindexPrev, stakeInput.get(), nBits, nTxNewTime, hashProofOfStake)) {
            ...
            nMasternodeReward = GetMasternodePayment(chainActive.Height() + 1 , nReward, 0, false);
            nCredit += nReward;

            // Create the output transaction(s)
            std::vector<CTxOut> vout;
            if (!stakeInput->CreateTxOuts(this, vout, nCredit - nMasternodeReward)) {
...
bool CWallet::MultiSend()
{
    ...
    for (const COutput& out : vCoins) {
        ...
        bool sendMSOnStake = fMultiSendStake && out.tx->IsCoinStake() && !outpoint.IsMasternodeReward(out.tx);

        // Don't send multiple multisend transactions after UTXO split
        ...
        CAmount nAmount = 0;
        CAmount nMultiSendAmount = out.tx->GetCredit(filter) - out.tx->GetDebit(filter);
        CAmount blockValue = GetBlockValue(chainActive.Height() - Params().COINBASE_MATURITY());
        CAmount masternodePayment = GetMasternodePayment(chainActive.Height() - Params().COINBASE_MATURITY(), blockValue, 0, false);

        // remove relevant portion of reward where we win entire block reward but only have partial multisend active
        if (nMultiSendAmount == blockValue) {
            if (!fMultiSendMasternodeReward) {
                nMultiSendAmount -= masternodePayment;
            }
            if (!fMultiSendStake) {
                nMultiSendAmount -= (blockValue - masternodePayment);
            }
        }

        for (unsigned int i = 0; i < vMultiSend.size(); i++) {
            // MultiSend vector is a pair of 1)Address as a std::string 2) Percent of stake to send as an int
            nAmount = (nMultiSendAmount * vMultiSend[i].second) / 100;
        ...
        if (fMultiSendNotify && nMultiSendAmount == blockValue) {
            stakeSent = true;
            mnSent = true;
