...
CMasternode::CollateralStatus CMasternode::CheckCollateral(const COutPoint& outpoint, int& nHeightRet)
{
    ...
    CCoins coins;
    if (!GetCoins(outpoint, coins)) {
        return COLLATERAL_NOT_FOUND;
    }
    ...
    if (!cm.CollateralValueCheck(coins.nHeight, coins.vout[outpoint.n].nValue)) {
...
bool CMasternode::CollateralValueCheck(int nHeight, CAmount TxValue)
{
    ...
    CAmount minCollateral = Params().MasternodeCollateralMinimum() * COIN;
    CAmount maxCollateral = Params().MasternodeCollateralMaximum() * COIN;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
...

    {
        ...
        if (CheckCollateral(vin.prevout) != CMasternode::COLLATERAL_OK) {
            return false;
        }
