...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered vpx address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal vpx:URI (vpx:XyZ...?message=example)
            {
...
static void NotifyzVPXReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzVPXReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzVPXReset.connect(boost::bind(NotifyzVPXReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzVPXReset.disconnect(boost::bind(NotifyzVPXReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zvpxTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
