...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a VPX address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no VPX: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert vpx:// to vpx:
    //
    //    Cannot handle this later, because vpx:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open vpx.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "VPX.lnk";
}
...
{
    return GetAutostartDir() / "vpx.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a vpx.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the vpx app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add vpx app to startup item list
