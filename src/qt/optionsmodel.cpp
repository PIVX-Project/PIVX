...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeVpxAmount"))
        settings.setValue("nAnonymizeVpxAmount", 1000);

    nAnonymizeVpxAmount = settings.value("nAnonymizeVpxAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeVpxAmount"))
        SoftSetArg("-anonymizevpxamount", settings.value("nAnonymizeVpxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in vpx.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeVpxAmount:
            return QVariant(nAnonymizeVpxAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeVpxAmount:
            nAnonymizeVpxAmount = value.toInt();
            settings.setValue("nAnonymizeVpxAmount", nAnonymizeVpxAmount);
            emit anonymizeVpxAmountChanged(nAnonymizeVpxAmount);
            break;
        case CoinControlFeatures:
