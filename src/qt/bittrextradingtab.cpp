...
BittrexTradingTab::BittrexTradingTab(QWidget *parent) :
    QWidget(parent),
    ...
    model(0)
{
    ...
    ui->DNETAvailableLabel->setTextFormat(Qt::RichText);
    ...
    ui->BittrexDNETLabel->setTextFormat(Qt::RichText);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "DNET(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "DNET(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    /*OrderBook Table Init*/
...
void BittrexTradingTab::UpdaterFunction(){
    //DNETst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString BittrexTradingTab::GetMarketHistory(){
      QString Response = sendRequest("https://bittrex.com/api/v1.1/public/getmarkethistory?market=BTC-DNET&count=100");
      return Response;
}
...
QString BittrexTradingTab::BuyDNET(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://bittrex.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-DNET&quantity=";
...
QString BittrexTradingTab::SellDNET(QString OrderType, double Quantity, double Rate){
...
QString BittrexTradingTab::GetOpenOrders(){
    QString URL = "https://bittrex.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-DNET";
...
QString BittrexTradingTab::GetDepositAddress(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=DNET";
...
QString BittrexTradingTab::GetAccountHistory(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-DNET&count=10";
...
int BittrexTradingTab::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>DNET Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> DNET");
...
void BittrexTradingTab::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    ui->DNETSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(DNETSupply,'i',8) + "</span><b> DNET</b>");
    ...
    ui->DNETDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(DNETDemand,'i',8) + "</span><b> DNET</b>");
...
void BittrexTradingTab::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
                case 0:    //buy tab is active
                    ...
                    Response2 = GetBalance("DNET");
                    Response3 = GetOrderBook();

                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BtcAvailableLabel, *ui->DNETAvailableLabel, Response, Response2);
                    }
                ...
                case 1: //Cross send tab active
                    ...
                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BittrexDNETLabel, *ui->BittrexBTCLabel, Response, Response2);
                    }
                ...
                case 5://show balance tab
                    ...
                    Response = GetBalance("DNET");
                    if(Response.size() > 0 && Response != "Error"){
                        DisplayBalance(*ui->DNETBalanceLabel,*ui->DNETAvailableLabel_2,*ui->DNETPendingLabel, QString::fromUtf8("DNET"),Response);
                    }
                break;
...
void BittrexTradingTab::CalculateCSReceiveLabel(){
    ...
    double AvailableDNET = BalanceObject["Available"].toDouble();
...
void BittrexTradingTab::on_CS_Max_Amount_clicked() {
    double Quantity = ui->BittrexDNETLabel->text().toDouble();
    double Received = 0;
    double Qty = 0;
    double Price = 0;
    QString buyorders = GetOrderBook();
...
void BittrexTradingTab::on_Withdraw_Max_Amount_clicked() {
    ...
    ui->WithdrawUnitsInput->setText(str.number(AvailableDNET,'i',8));
}
...
void BittrexTradingTab::on_BuyDNET_clicked(){
    double Rate;
    double Quantity;
...
void BittrexTradingTab::on_SellDNETBTN_clicked(){
    double Rate;
    double Quantity;
...
void BittrexTradingTab::on_CSUnitsBtn_clicked(){
   ...
   bool relock = true;
    ...
    WalletModel::UnlockContext ctx(model->requestUnlock(relock));
    if(!ctx.isValid())
    {
        ...
        Q_FOREACH (const QJsonValue & value, BuyArray)
        {
            ...
            if ( ((Quantity / x) - y) > 0 )
            {
                ...
                QString SellResponse = SellDNET(Order,y,x);
                ...
                QString SellResponse = SellDNET(Order,(Quantity / x),x);
                QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
                QJsonObject SellResponseObject = SelljsonResponse.object();                              //get json obj
                ...
                if (SellResponseObject["success"].toBool() == false){
                    ...
                    if (ResponseObject["success"].toBool() == false){
                        ...
                        if (ResponseObject["success"].toBool() == false){
                            ...
                            QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" DNET for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                        }
                    } else if (ResponseObject["success"].toBool() == true){
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" DNET for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
...
void BittrexTradingTab::on_UnitsInputDNET_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
