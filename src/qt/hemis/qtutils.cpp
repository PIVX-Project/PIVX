...
void setSortTxTypeFilter(QComboBox* filter, SortEdit* lineEditType)
{
    ...
    filter->addItem(QObject::tr("GM reward"), TransactionFilterProxy::TYPE(TransactionRecord::GMReward));
...
{
    if (isSelected) {
        return QColor("#25c2bfc8");
    } else if (isHovered) {
        return QColor("#25bababa");
    } else {
        return isLightTheme ? QColor("#ffffff") : QColor("#101011");
    }
}
