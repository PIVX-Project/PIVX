// Copyright (c) 2017-2018 The XVIP developers
...
CzSYNXWallet::CzSYNXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zsynx seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSYNXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSYNXSeed_deprecated()) {
                    LogPrintf("%s: Updated zSYNX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zsynx seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zsynx wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSYNXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSYNXCount(nCountLastUsed);
    else if (!walletdb.ReadZSYNXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSYNXWallet::Lock()
{
    seedMaster = 0;
}

void CzSYNXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSYNXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSYNX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSYNXWallet::LoadMintPoolFromDB()
{
...
void CzSYNXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSYNXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSYNXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsynxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSYNXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSYNX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsynxTracker which also adds to database
    pwalletMain->zsynxTracker->Add(dMint, true);
...
void CzSYNXWallet::SeedToZSYNX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSYNXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSYNXWallet::GenerateDeterministicZSYNX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSYNXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSYNXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
