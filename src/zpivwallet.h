...
#ifndef SYNX_ZSYNXWALLET_H
#define SYNX_ZSYNXWALLET_H
...
public:
    CzSYNXWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSYNX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSYNX(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SYNX_ZSYNXWALLET_H
