VPX Core version *3.1.0* is now available from:  <https://github.com/vpx-project/vpx/releases>
...
VPX Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
VPX Core is extensively tested on multiple operating systems using
...
VPX Core should also work on most other Unix-like systems but is not
...
zVPX Staking is here! zVPX staking will be activated on the morning of the 8th of May 2018. With the release of zVPX staking, there are effectively 2 versions of zVPX, zVPX minted on the 3.0.6 VPX wallet or lower, and zVPX minted on VPX wallet version or higher. New features in this release will require the use of zVPX v2, zVPX minted on this wallet release 3.1.0 or later. If you currently hold zVPX v1 and wish to take advantage of zVPX staking and deterministic zVPX, you will need to spend the zVPX v1 to yourself and remint zVPX v2.
Note: To find your zVPX version, click the privacy tab, then the zVPX Control button then expand the arrows next to the desired denomination.


### Deterministic zVPX Seed Keys

zVPX is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zVPX outside of the wallet that the zVPX had been minted on. zVPX can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zVPX minting

zVPX minting now only requires 1 further mint (down from 2) to mature. zVPX mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zVPX Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zVPX denomination has been spent or not.



VPX/zVPX Staking and Masternode Rewards
--------------

### VPX, zVPX and Masternode Payment Schedule

To encourage the use of zVPX and increase the VPX zerocoin anonymity set, the VPX payment schedule has been changed to the following:

If a user staking zVPX wins the reward for their block, the following zVPX reward will be: 
- 3 zVPX (3 x 1 denominations) rewarded to the staker, 2 VPX rewarded to the masternode owner and 1 VPX available for the budget. This is a total block reward of 6 VPX, up from 5.

If a user staking VPX wins the reward, the following amounts will be rewarded: 
- 2 VPX to the VPX staker, 3 VPX to the Masternode owner and 1 VPX available for the budget. This is a total block reward of 6 VPX, up from 5.


### Return change to sender when minting zVPX

Previously, zVPX minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most VPX and return the change from a zVPX mint to that address.
...
The visual layout of the VPX Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zVPX minting feature. This is enabled by default and the enablezeromint=0 setting in the vpx.conf file will overwrite the GUI option.
-	The percentage of autominted zVPX can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular VPX.
...
VPX Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The VPX client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running privixd without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the VPX support group, which often times sees users say "I didn't spend that zVPX". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the VPX masternodes, the monthly budget available to be utilised has been increased to 42,000 VPX / month. This VPX only has the opportunity to be raised once per month (paid to winning proposals) with any unused VPX not created by the blockchain.

### Budget Finalization Fee

The VPX finalization fee for successful proposals has now been reduced, this fee is now 5 VPX down from 50 VPX. The total fee outlay for a successful proposal is now a total of 55 VPX.
...
-	Enforced v1 zVPX spends to require a security level of 100
-	Updates to zVPX spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the VPX-Qt masternodes tab
-	Updated VPX-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and VPX's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zVPX Control dialog in the QT wallet, a user is able to see zVPX marked as version 1 or 2.
...
Zerocoins, or zVPX, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zVPX that can be recalculated at any time using the seed. Deterministic zVPX allows for users to backup all of their future zVPX by simply recording their seed and keeping it in a safe place (similar to backing up a private key for VPX). The zVPX seed needs to remain in the wallet in order to spend the zVPX after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zVPX data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

VPX's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which VPX used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to VPX using the bug bounty program. The modulus is now correctly imported and VPX's accumulators have been changed to use the new proper modulus.
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zVPX balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zVPX status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zVpx Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault privix-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zVPX spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zVPX. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zVPX. (presstab)
