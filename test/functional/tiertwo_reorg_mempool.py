...
class TiertwoReorgMempoolTest(PivxTestFramework):
    ...
    def register_masternode(self, from_node, dmn, collateral_addr):
        dmn.proTx = from_node.fund_protx_registration(collateral_addr, dmn.ipport, dmn.owner,
    ...
    def run_test(self):
        ...
        self.register_protx_ext(nodeA, nodeA, mempool_dmn4, mempool_dmn4.collateral, True)

        # Now send a valid proUpServ tx to the mempool, without mining it
        proupserv1_txid = nodeA.update_protx_service(pre_split_mn1.proTx,
        ...
        assert_raises_rpc_error(-26, "protx-dup", nodeA.update_protx_service,
                                mnsA[0].proTx, "127.0.0.1:1000", "", mnsA[0].operator_sk)

        # Now send other two valid proUpServ txes to the mempool, without mining them
        proupserv2_txid = nodeA.update_protx_service(mnsA[3].proTx,
                                                     "127.0.0.1:2000", "", mnsA[3].operator_sk)
        proupserv3_txid = nodeA.update_protx_service(pre_split_mn1.proTx,
        ...
        proupreg1_txid = nodeA.update_protx_registrar(mnsA[4].proTx, operator_to_reuse, "", "")
        ...
        assert_raises_rpc_error(-26, "protx-dup", nodeA.update_protx_registrar,
                                mnsA[5].proTx, mempool_dmn1.operator_pk, "", "")

        # Now send other two valid proUpServ txes to the mempool, without mining them
        new_voting_address = nodeA.getnewaddress()
        proupreg2_txid = nodeA.update_protx_registrar(mnsA[5].proTx, "", new_voting_address, "")
        proupreg3_txid = nodeA.update_protx_registrar(pre_split_mn1.proTx, "", new_voting_address, "")

        # Send two valid proUpRev txes to the mempool, without mining them
        self.log.info("Revoking two masternodes...")
        prouprev1_txid = nodeA.revoke_protx(mnsA[6].proTx, mnsA[6].operator_sk)
        prouprev2_txid = nodeA.revoke_protx(pre_split_mn2.proTx, pre_split_mn2.operator_sk)
