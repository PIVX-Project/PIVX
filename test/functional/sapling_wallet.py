...
class WalletSaplingTest(PivxTestFramework):
    ...
    def run_test(self):
        ...
        shieldSpend = resp['vShieldSpend'][0]
        assert('cv' in shieldSpend)
        assert('anchor' in shieldSpend)
        assert('nullifier' in shieldSpend)
        assert('rk' in shieldSpend)
        assert('proof' in shieldSpend)
        assert('spendAuthSig' in shieldSpend)
        shieldOutput = resp['vShieldOutput'][0]
        assert('cv' in shieldOutput)
        assert('cmu' in shieldOutput)
        assert('ephemeralKey' in shieldOutput)
        assert('encCiphertext' in shieldOutput)
        assert('outCiphertext' in shieldOutput)
        assert('proof' in shieldOutput)
